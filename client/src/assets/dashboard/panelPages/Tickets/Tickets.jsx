...
export default function Tickets() {
  ...
  const getAllTickets = () => {
    ...
    if (isTokenExpired(userToken.accessToken)) {
      ...
      fetch(`${baseURL}/tickets/open`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${userToken.accessToken}`
        }
      }).then(res => res.json()).then(data => { setTicketList(data); setIsContentReady(true); console.log(data); })
    }
  }
  ...
  const sendNewMessage = (chat_id) => {
    ...
    if (isTokenExpired(userToken.accessToken)) {
      ...
      fetch(`${baseURL}/tickets/${currentChat._id}/reply`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${userToken.accessToken}`,
        ...
        <tbody>
          {
            ticketList?.reverse().map((ticket, i) => (
              <tr key={i}>
                ...
                <td>{ticket.status === "OPEN" && (<><RiKakaoTalkFill onClick={() => { setShowChatBox(true); setCurrentChat(ticket) }}/><FaPowerOff onClick={resolveHandler} style={{color: "red", marginLeft: 10}}/></>)}</td>
              </tr>
            ))
          }
        </tbody>
              ...
              <div className={s.messages_filed}>
                {
                  currentChat.chat.map((item, i) => (
                    <div className={`${s.chat_row} ${item.role !== "CUSTOMER" ? s.myMessage : s.customer}`} key={i}>
