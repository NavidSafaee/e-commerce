  ...
  return (
    <section className={style.profileMainSection}>
      ...
      <div className={style.filedsWrapper}>
        ...
        <div className={style.editItem}>
          <div className={style.contentSide}>
            {userInfo?.email ? <Tooltip title="verified" placement="right" TransitionComponent={Zoom}><b className={style.boxTitle}>Email <MdVerified /></b></Tooltip> : <b className={style.boxTitle}>Email</b>}
            <span className={style.boxValue}>{userInfo?.email}</span>
          </div>
          <RiEdit2Line className={style.editBtn} onClick={() => {setModalType("email"); setDefault_value((userInfo?.email) ? userInfo?.email : "")}}/>
          {!userInfo?.email ? <Tooltip title="Please fill in this field" placement="top" TransitionComponent={Zoom}><Button><PiWarningFill className={style.warnIcon} /></Button></Tooltip> : null}
        </div>
        <div className={style.editItem}>
          <div className={style.contentSide}>
            {userInfo?.phoneNumber ? <Tooltip title="verified" placement="right" TransitionComponent={Zoom}><b className={style.boxTitle}>Phone number <MdVerified /></b></Tooltip> : <b className={style.boxTitle}>Phone number</b>}
            <span className={style.boxValue}>{userInfo?.phoneNumber ? userInfo?.phoneNumber : "---"}</span>
          </div>
          <RiEdit2Line className={style.editBtn} onClick={() => {setModalType("phoneNumber"); setDefault_value((userInfo?.phoneNumber) ? userInfo?.phoneNumber : "")}}/>
          {!userInfo?.phoneNumber ? <Tooltip title="Please fill in this field" placement="top" TransitionComponent={Zoom}><Button><PiWarningFill className={style.warnIcon} /></Button></Tooltip> : null}
        </div>
        <div className={style.editItem}>
          <div className={style.contentSide}>
          ...
          <RiEdit2Line className={style.editBtn} onClick={() => {setModalType("birthday"); setDefault_value((userInfo?.birthDate) ? userInfo?.birthDate.slice(0, 10) : "")}}/>
          {!userInfo?.birthDate ? <Tooltip title="Please fill in this field" placement="top" TransitionComponent={Zoom}><Button><PiWarningFill className={style.warnIcon} /></Button></Tooltip> : null}
        </div>
        <div className={style.editItem}>
          <div className={style.contentSide}>
