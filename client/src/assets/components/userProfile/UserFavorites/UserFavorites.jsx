...
import s from './UserFavorites.module.scss'
  ...
  const getAllFavorites = () => {
    ...
    if (isTokenExpired(userToken.accessToken)) {
      ...
      fetch(`${baseURL}/reviews/me`, {
        ...
        .then(data => { setMyFavorites(data); setIsContentReady(true) })
    }
  }
        ...
        isContentReady && (
          <section className={s.favoritesection}>
            {myFavorites?.length === 0 ? (
              <div className={s.emptyBox}>favorites</div>
            )
              :
              (
                <>
                  <div className={s.favoritesContainer}>
                    {myFavorites?.reverse().map((comment, i) => (
